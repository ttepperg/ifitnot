...
!        Physical parameters (cgs UNITS)
        ...
        real(kind=doubleR), parameter :: CLIGHT=2.9979d+10
        real(kind=doubleR), parameter :: ERADIUS=2.817d-13
        real(kind=doubleR), parameter :: PI=3.141593d0
        real(kind=doubleR), parameter :: ECHARGE=1.602d-19
        real(kind=doubleR), parameter :: EMASS=9.109d-34

!        Atomic numbers
        integer, parameter :: A_H=1, A_He=4, A_O=16, A_Ne=20, A_C=12, A_Si=28

        real(kind=doubleR), parameter :: H_MASS = 1.673d-27 !kg
        real(kind=doubleR), parameter :: boltzmann=1.3806504d-23 !J/K

        real(kind=doubleR) :: lambda_0, fvalue, gamma
...
!       ------------------------------------------------------------------------
        ...
        integer(kind=singleI) :: counter, indx, size, nveloc
...
!        real(kind=doubleR) :: lambda_0, fvalue, gamma, m_ion
        real(kind=doubleR) :: m_ion
        real(kind=doubleR) :: equiv_width_mA, eqw_aux_nc, eqw_aux_cv
        real(kind=doubleR) :: void, temp
        real(kind=doubleR) :: b_value, logNion, Nion, dopp_width, damping,
     +  sqrtPI
...
!        character(len=24) :: ion_str
        character(len=256) :: homedir
        character(len=256) :: infile, outfile_alp, outfile_ifit, outfile_prefix,
     +  plotfile, psfile
        character(len=256) :: command
        character(len=10) :: signaltonoise_str, plot_str, fwhm_str, verbose_str
        character(len=5) :: profile_str

        character(len=256) :: errmsg, warnmsg, statmsg
        
!        logical :: ion_found
        logical :: ion_found, addnoise, verbose
...
!       In red=31
        errmsg=CHAR(27)//'[1;31mERROR: abs_line_profile ...'//
     &  CHAR(27)//'[0m'
!       In yellow=33
        warnmsg=CHAR(27)//'[1;33mWARNING: abs_line_profile ...'//
     &  CHAR(27)//'[0m'
!       In green=32
        statmsg=CHAR(27)//'[1;32mSTATUS: abs_line_profile ...'//
     &  CHAR(27)//'[0m'
...
!       ------------------------------------------------------------------------
        ...
        if (argcount .ne. 8) then
         write(6,*) 'USAGE: abs_line_profile <transition> <log10 Nion> '//
     &   '<b (km/s)> <noise (S/N or inf)> <profile (voigt | gauss) '//
     &   ' <FWHM (0. for no convolution)> <plot (T/F)> <output info (T/F)'
         stop 1
        end if
        ...
        read (fwhm_str,'(f5.2)') fwhm_kms

        call GetArg(7,plot_str)
        
        call GetArg(8,verbose_str)
        verbose = .true.
        if (verbose_str.eq.'F') verbose = .false.
...
!       ------------------------------------------------------------------------
!       Define wavelength range in terms of the equivalent width
!       Include FWHM criterion!
...
!       Compute equivalent width using external function 'eqw'
     ...
     &  //' > output_data/eqw_'//
     &  trim(ion_name)//'_'//trim(logNion_str)//'_'//trim(b_value_str)
     &  //'.dat'
        call system(trim(command))

!       Read equivalent width from file eqw.dat
        open(50, file='output_data/eqw_'//
     &  trim(ion_name)//'_'//trim(logNion_str)//'_'//trim(b_value_str)
     &  //'.dat', status='old')
...
!       Define wavelength range (5x the equivalent width in Angstroem)
        wl_range = 1.0d0*ceiling(5.0d-3*equiv_width_mA)

!       Compute pixel size in Angstroem
!        pixel_size_Ang = (pixel_size_kms * lambda_0) / (CLIGHT*1.0d-5)
        pixel_size_Ang =
     &  lambda_0 * (dexp(pixel_size_kms / (CLIGHT*1.0d-5)) - 1.0d0)
     ...
     &   trim(ion_name), lambda_0, fvalue, gamma, logNion, b_value,
...
!       Define ion mass
        if (ion_name(1:3).eq.'o6') then
         m_ion = A_O*H_MASS
        else if (ion_name(1:6).eq.'ne8') then
         m_ion = A_Ne*H_MASS
        else if (ion_name(1:10).eq.'si3') then
         m_ion = A_Si*H_MASS
        else if (ion_name(1:3).eq.'c4') then
         m_ion = A_C*H_MASS
        else if (ion_name(1:2).eq.'h1') then
         m_ion = H_MASS
        else
         write(6,*) trim(warnmsg)//'m_ion not yet defined for other'
     &   //' ions than h1, o6, si3, and ne8'
         stop 1
        end if
...
!       ------------------------------------------------------------------------
!       calculate optical depth as a function of wavelength
!       ------------------------------------------------------------------------
!       Define constants
        sqrtPI = dsqrt(PI)
...
!       Central optical depth
        tau_central = (sqrtPI*ERADIUS*fvalue*(lambda_0*lambda_0)*
...
!       optical depth; check for numerical precision
     ...
     &    profile_func =
     &    voigt_hjerting(damping,b_value,lambda_0,wavelength(counter))
         
         if (profile_str.eq.'gauss')
     &    profile_func = gauss(b_value,lambda_0,wavelength(counter))
...
!       set output filename and open

        outfile_prefix = './output_data/abs_line_profile_'//
     &  trim(logNion_str)//'_'//trim(b_value_str)//'_'//
     &  trim(signaltonoise_str)//'_'//trim(profile_str)//'_'//trim(ion_name)

!       1: to be shown graphically
        outfile_alp = trim(outfile_prefix)//'.alp'

!       2: used for ifitnot benchmarking
...
!       Compare both measurements (should agree within 1%)
        if ((verbose).and.
     &   (((equiv_width_mA-eqw_aux_nc)/equiv_width_mA)*1.0d2.gt.1.0d0)) then
         write(6,*) trim(warnmsg)//'Non matching equivalent'
     &  //' widths: ', equiv_width_mA, 'vs.', eqw_aux_nc,
     &  ' between CoG and direct integration!'
!             stop 1
        end if
...
!       Compare both measurements (should agree within 1%)
        if ((verbose).and.
     &   (((eqw_aux_cv-eqw_aux_nc)/eqw_aux_nc)*1.0d2.gt.1.0d0)) then
         write(6,*) trim(warnmsg)//'Non matching equivalent'
     &  //' widths: ', eqw_aux_cv, 'vs.', eqw_aux_nc,
     &  ' between convolved and non-convolved flux!'
!             stop 1
        end if
...
!       ----------------------------------------------------------------
!       Equivalent width for lines on the linear part of the curve of growth
        if (verbose) then
         write(6,*) 'Equivalent Width (mA; linear portion of CoG) Error (%)'
         write(6,*) PI*(ERADIUS*1.d11)*fvalue*(lambda_0*lambda_0*1.d-16)*Nion,
     &   abs(1.0d0 - PI*(ERADIUS*1.d11)*fvalue*(lambda_0*lambda_0*1.d-16)*Nion
     &   / eqw_aux_nc)*1.d2
        end if

!       ----------------------------------------------------------------
!       Equivalent width for lines on the flat part of the curve of growth
        if ((verbose).and.(tau_central.gt.1.0d0)) then
         write(6,*) 'Equivalent Width (mA; flat portion of CoG) Error (%)'
         write(6,*) 2*lambda_0*1.d3*(b_value*1.d5/CLIGHT)*sqrt(log(tau_central)),
     &   abs(1.0d0 - 2*lambda_0*1.d3*(b_value*1.d5/CLIGHT)*sqrt(log(tau_central))
     &   / eqw_aux_nc)*1.d2
        end if
!       ----------------------------------------------------------------
!       Create gnuplot file
...
!       set psfile name
        psfile=trim(outfile_prefix)//'_alp.ps'

!       set plotfile name
        plotfile=trim(outfile_prefix)//'_alp.plot'
...
!       ========================================================================
        subroutine load_ion(ion_str)
        use set_precision
        use physical, only: lambda_0, gamma, fvalue

        implicit none

        character(len=*), intent(in) :: ion_str
...
!       strongest transition ONLY (for now!)
         ...
         case default
          write(6,*) 'ERROR: Non-valid ion: '//trim(ion_str)
          stop 1
...
!       the corresponding transition.
        ...
        real(kind=doubleR), parameter :: PI=3.141593d0, CLIGHT=2.9979d+10
...
!       GENERATES UNIFORMILY DISTRIBUTED RANDOM NUMBERS
        ...
        integer(kind=singleI), parameter :: IA=16807,IM=2147483647,IQ=127773,IR=2836,
     !  NTAB=32
        real(kind=doubleR), parameter :: AM=1.0d0/dble(IM),
     !  NDIV=1+(IM-1)/NTAB,EPS=1.2d-7,RNMX=1.0d0-EPS
...
!       fwhm_kms = FWHM of Gaussian response in km/s
     ...
     &  convl_flux_convolved(:)
        ...
        write(6,'(a,f7.2)')
     &  'Convolving with Gaussian LSF with FWHM [km/s]:', fwhm
        write(6,*)
