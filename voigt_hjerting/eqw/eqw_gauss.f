...
!       ----------------------------------------------------------------
        ...
        double precision :: exp_gauss_x
        logical :: flag
        external :: exp_gauss_x
...
!       In red=31
        errmsg=CHAR(27)//'[1;31mERROR: eqw_gauss ...'//
     &  CHAR(27)//'[0m'
!       In yellow=33
        warnmsg=CHAR(27)//'[1;33mWARNING: eqw_gauss ...'//
     &        CHAR(27)//'[0m'
!       In green=32
        statmsg=CHAR(27)//'[1;32mSTATUS: eqw_gauss ...'//
     &  CHAR(27)//'[0m'
...
!       and gamma-value (in Hz) of the corresponding transitions
        ...
        flag=.false.
        open(20, file=trim(homedir)//'/ifitnot/voigt_hjerting/eqw/ions.dat')
         do 30
...
!       write(6,*) ion_str, lambda_0, fvalue, gamma
...
!       factor 1.0d5 to transform b [km/s] -> b [cm/s]
...
!       exp_h_ax (see ~/ifitnot/voigt_hjerting/misc/numeric)

!       Integrate the flux decrement, i.e. 1-exp(-tau), over lambda
!       from 0 to wl_range; equivalent width is given by 2x this integral

        width = 0.0d0
        call integrate(exp_gauss_x,0.0d0,wl_range,1.0d-8,width)
        write(6,'(f6.2,2x,f6.2,2x,f14.4)') DLOG10(N_ion), b, 2.0*width*1.0d3
!       width = 0.0d0
!       call qromb(exp_gauss_x,0.0d0,wl_range,width)
!       write(6,'(f6.2,2x,f6.2,2x,f14.4)') DLOG10(N_ion), b, 2.0*width*1.0d3
!       width = 0.0d0
!       call qsimp(exp_gauss_x,0.0d0,wl_range,width)
!       write(6,'(f6.2,2x,f6.2,2x,f14.4)') DLOG10(N_ion), b, 2.0*width*1.0d3
